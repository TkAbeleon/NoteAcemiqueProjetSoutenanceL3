...
**Membre 1 — Developpeur Frontend** : integralite de l'application Expo/React Native Web, navigation expo-router, lecteurs video/audio expo-av, mini-player persistant, formulaires, tableau de bord admin et interface fournisseur, integration Stripe Elements. Memoire principal : plateforme collaborative de gestion d'idees innovantes.

**Membre 2 — Developpeur Backend** : integralite de l'API REST Node.js/Express, modeles Mongoose, Multer, extraction metadonnees audio, integration Stripe SDK, tests fonctionnels Postman. Memoire principal : systeme de diagnostic medical assiste.

**Membre 3 — Responsable Securite et Chef de Projet Fonctionnel** : strategie de securite transversale (JWT, bcrypt, CORS, rate limiting, helmet, validation), tests de securite, configuration Nginx/SSL, coordination, maquettes Figma, redaction complete de la documentation et du memoire, preparation soutenance. Memoire principal : plateforme IA de suivi des feux de brousse et de la deforestation a Madagascar (donnees NASA MODIS/VIIRS, IA classique et generative).
